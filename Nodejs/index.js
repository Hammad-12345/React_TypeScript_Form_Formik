...
app.post("/Add_data", async(req, res) => {
  const { firstname,lastname,fathername,cnicno,mobileno,password,gender } = req.body
  try {
    // const saltRound = 10;
...
app.put("/Update_Data", async(req, res) => {
  const { firstname,lastname,fathername,cnicno,mobileno,password,gender } = req.body.value
  const {id}=req.body
  try {
    const update = await profileschema.updateOne({_id:id},{
      $set:{
        firstname,
        lastname,
        fathername,
